...
// Por ahora usamos el primer asesorado activo como demo
const client = mockClients.find((c) => c.status === 'active') || mockClients[0]
const lastCheckin = mockCheckins.find((c) => c.clientId === client.id)

const nextCheckinDate = new Date(client.nextReview)
const today = new Date()
const daysUntilCheckin = Math.ceil((nextCheckinDate - today) / (1000 * 60 * 60 * 24))
    ...
    <div className="min-h-screen bg-[#0a0a0f] pb-10">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 flex flex-col gap-5">
        ...
        <SectionCard
          ...
          subtitle={`Actualizado ${new Date(client.nutrition.lastUpdate).toLocaleDateString('es-AR', { day: 'numeric', month: 'long' })}`}
          action={
            <button
